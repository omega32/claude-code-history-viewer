...
- User-authorship provenance advanced the opaque Codex snapshot cursor version to `3`; zero-output token-bookkeeping handling advances it to `4`. Older cursors fall back to a complete snapshot so retained prefixes normalized under an earlier contract are repaired rather than reused.
