...
- User-authorship provenance advanced the opaque Codex snapshot cursor version to `3`; zero-output token-bookkeeping handling advanced it to `4`; prompt-artifact metadata advanced it to `5`; physical-turn/client provenance and strict terminal-sequence invalidation advanced it to `6`; interleaved developer context advanced it to `7`; overlapping-task authorship isolation advanced it to `8`; app-server-compatible positive user projection advanced it to `9`; provider-neutral interruption events advanced it to `10`; provider-rendered images advanced it through `12`; fork rollback provenance advanced it to `13`; confirmed zero-exclusion fork boundaries advanced it to `14`; asynchronous question reply metadata advances it to `15`. Older cursors fall back to a complete snapshot so retained prefixes normalized under an earlier contract are repaired rather than reused.
