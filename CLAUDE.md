...
Codex fork rollback provenance is classified during the existing rollout parse. After the first child `session_meta` establishes a valid `forked_from_id`, replayed non-child metadata opens the inherited-history phase; only a positive `thread_rolled_back` followed by a replacement `task_started` and then the first return to the child session id gains additive `data.rollbackOrigin:"fork"`. A replayed historical rollback is cleared by intervening non-child metadata or terminal task activity, and any later same-session rollback remains unclassified. An unconfirmed incremental suffix is deliberately checkpointed from before its candidate so later child metadata replays and repairs it. Snapshot cursor version 13 invalidates retained normalized prefixes that cannot carry this classification.
...
- User-authorship provenance advanced the opaque Codex snapshot cursor version to `3`; zero-output token-bookkeeping handling advanced it to `4`; prompt-artifact metadata advanced it to `5`; physical-turn/client provenance and strict terminal-sequence invalidation advanced it to `6`; interleaved developer context advanced it to `7`; overlapping-task authorship isolation advanced it to `8`; app-server-compatible positive user projection advanced it to `9`; provider-neutral interruption events advanced it to `10`; provider-rendered images advanced it through `12`; fork rollback provenance advances it to `13`. Older cursors fall back to a complete snapshot so retained prefixes normalized under an earlier contract are repaired rather than reused.
