...
- Claude headless dumps read only the selected JSONL carrier. They preserve `isSidechain` on records already present in that carrier, so directly dumping a discovered subagent file lets a consumer include or exclude its sidechain records; dumping the parent session does not discover or merge sibling `<session-id>/subagents/*.jsonl` files. The Tauri-only `get_session_subagents` command already discovers regular and workflow subagent transcripts and exposes `agent_id`, `file_path`, timestamps, optional exact spawning `tool_use_id` from `agent-*.meta.json`, and optional `workflow_run_id`, but that relation is not yet part of the headless protocol. Parent-side transcript merging and child file-fact folding remain deferred consumer work; do not describe complete headless loading as globally pre-filtering sidechains.
