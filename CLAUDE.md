...
The headless `image-artifacts-v1` capability preserves two presentation proofs without treating arbitrary tool images as chat output. Final-Markdown images retain the existing exact provider-turn, tool-result byte identity, and local-path correlation chain. Narrowly parsed `Promise.all` exec shapes additionally map each literal `tools.view_image` call to its output when either named destructured bindings are emitted individually through `image(variable.image_url)` or one result-array binding is emitted in order through an exact `for (const item of results) image(item.image_url)` loop; unmatched counts, duplicate variables, nonliteral paths, reordered or transformed collections, blocks, trailing code, and all unrecognized JavaScript are rejected. Completed `event_msg.image_generation_end` records are normalized separately as `provider_rendered_image` Canvas artifacts whose base64 source, call ID, media identity, and presentation kind remain in one message. Because those events currently carry no turn ID, they are emitted only while exactly one non-ambiguous task owns the event; overlap omits them. Authored Codex prompt images retain source semantics independently: URL/data carriers normalize as `source.type:"url"`, while legacy `local_images[]` and paginated `local_image.path` carriers normalize as `source.type:"path"` so Windows and POSIX paths are never presented as URL schemes. Snapshot cursor version 17 invalidates retained prefixes that predate the corrected local-image locator kind.
