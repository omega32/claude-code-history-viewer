...
### Headless interruption normalization

The headless dump contract stamps provider-generated terminal cancellation as `subtype:"interruption"`. Claude classification is deliberately limited to an exact whole user-role payload of `[Request interrupted by user]` or `[Request interrupted by user for tool use]` (scalar text or one text block); it adds `data.reason:"interrupted"`, excludes the record from conversational counts/title fallbacks while retaining it as session activity, and uses metadata cache version 17. Embedded, extended, or multi-block quotations do not qualify, but Claude records no independent provenance capable of distinguishing an authored message whose complete content equals one sentinel. Codex converts authoritative `event_msg.turn_aborted` to the same subtype and `[interrupted]` content while retaining `data.providerTurnId`, reason, and available timing; the abort terminalizes that exact task lane, and snapshot cursor version 10 prevents older prefixes from omitting the event. Do not infer interruption from arbitrary quoted prose or Codex's raw `<turn_aborted>` model-context carrier.

### Codex image artifact normalization

The headless `image-artifacts-v1` capability preserves presentation and tool-result image proofs without treating arbitrary tool images as chat output. Final-Markdown images retain the existing exact provider-turn, tool-result byte identity, and local-path correlation chain. Narrowly parsed `Promise.all` exec shapes additionally map each literal `tools.view_image` call to its output when either named destructured bindings are emitted individually through `image(variable.image_url)` or one result-array binding is emitted in order through an exact `for (const item of results) image(item.image_url)` loop; unmatched counts, duplicate variables, nonliteral paths, reordered or transformed collections, blocks, trailing code, and all unrecognized JavaScript are rejected. Completed `event_msg.image_generation_end` records are normalized separately as `provider_rendered_image` Canvas artifacts whose base64 source, call ID, media identity, and presentation kind remain in one message. Because those events currently carry no turn ID, they are emitted only while exactly one non-ambiguous task owns the event; overlap omits them. Generic `tool_result_image` descriptors gain optional `producer:{kind:"mcp",server,tool}` metadata from either one direct `mcp__*` function namespace or one uniquely correlated completed `McpToolCall` image in the same provider turn; exact call identity wins over byte correlation, while conflicting call evidence or the same bytes from distinct producers omits the producer instead of guessing. Nested `exec` and deferred `wait` delivery therefore retain the actual inner MCP producer rather than the outer envelope. Authored Codex prompt images retain source semantics independently: URL/data carriers normalize as `source.type:"url"`, while legacy `local_images[]` and paginated `local_image.path` carriers normalize as `source.type:"path"` so Windows and POSIX paths are never presented as URL schemes. Snapshot cursor version 18 invalidates retained prefixes that predate producer attribution and the corrected local-image locator kind.
...
- `--dump-session-snapshot <session-id|session-path> [--provider <name>] [--cursor <opaque-token>] [--format json] [--output <file>]` — additive cursor-aware normalized refresh surface, advertised as `dump-session-snapshot` by `--capabilities`. With no cursor, providers return a discriminated `kind:"full"` envelope. A cursor-bearing `full` or `replace` also emits `cursorReplaceFrom`, the authoritative normalized boundary owned by the returned cursor; on `replace`, the existing `replaceFrom` remains the boundary applied by that response, so consumers can apply the current suffix and compact retained state for the next one without decoding the opaque cursor. Codex plain rollouts can return `kind:"unchanged"` or an exact `kind:"replace"` suffix after validating an opaque provider-owned cursor, hashing the complete previously accepted source prefix, and resuming from a completed-turn parser checkpoint. Claude does the same from a verified ordinary-authored-user boundary with explicit late tool-result and backward-parent fallback. Copilot CLI/Desktop and VS Code first replay and finalize the complete authoritative source, then return `unchanged` or a suffix only when the previous cursor's retained normalized-prefix digest is byte-identical; arbitrary earlier patches, late tool completion, parent-chain changes, checkpoint regression, and cursor/path/surface mismatch return a complete result. Copilot therefore avoids retained-prefix transfer and downstream reconstruction but does not yet skip provider replay. Rewrites, shrinks, incompatible cursors, compressed rollouts, unsafe backward references, and uncertain reads return an ordinary successful full result with a diagnostic `reason`; they do not weaken verification. The existing `--dump-session` bare-array contract is unchanged.
- `--audit-codex-authorship <absolute-rollout-path> --app-server-response <absolute-json-path> [--format json] [--output <new-file>]` is an opt-in differential check advertised by `--capabilities`. It compares the normal Codex parser's ordered physical turn identities and authored `(client identity, turn identity, authored_user/steer)` projection with a separately obtained app-server `thread/read` response. Exit 0 and `status:"match"` require a `schemaVersion:1` capture shaped as `{"schemaVersion":1,"rollout":{"threadId":"...","sha256":"...","length":123},"appServer":{"cliVersion":"...","response":{...}}}` whose asserted thread id, exact rollout byte length, and SHA-256 match the stable file read by the audit; every turn must also carry explicit `itemsView:"full"` and a concrete `items` array. Bare result/JSON-RPC captures, changing rollouts, active or incomplete turns, missing/duplicate identities, unfamiliar app-server item types, and authorship-corridor protocol-drift diagnostics are inconclusive and exit 1. Requiring `itemsView:"full"` is deliberately stricter than the general `thread/read` contract: an implementation that omits it may produce a safe false-negative rather than an unverifiable match. The envelope is an integrity binding, not a capture-time attestation: obtain it through a trusted collector that hashes the rollout immediately before and after `thread/read`, and treat a collector that merely wraps an older response as untrusted. Both inputs must be absolute regular paths without symlink/reparse components, the rollout must be a currently discoverable Codex carrier under the active or archived roots, and the response is size-bounded before and after reading. Reports emit only opaque SHA-256 identity and validated-CLI-version digests, counts, source line numbers, and typed reason codes—never provider-controlled strings, transcript content, raw payloads, hashes from the capture, or either input path. `--output` uses atomic create-new semantics and refuses every existing path, including either input, so the audit cannot overwrite source evidence. Ordinary dumps, snapshots, caches, and UI behavior do not consume this sidecar.
- Claude headless dumps read only the selected JSONL carrier. They preserve `isSidechain` on records already present in that carrier, so directly dumping a discovered subagent file lets a consumer include or exclude its sidechain records; dumping the parent session does not discover or merge sibling `<session-id>/subagents/*.jsonl` files. The read-only `--list-session-subagents <session-id|session-path> --provider claude` headless command reuses `get_session_subagents` discovery and emits regular and workflow child metadata. A regular child gains `spawned_at` only when its sibling `agent-*.meta.json` supplies a non-empty `toolUseId` that uniquely matches an `Agent` or legacy `Task` tool call in the normalized parent; missing, malformed, wrong-tool, or duplicate evidence leaves it unset. The command also retains `agent_id`, `file_path`, child timestamps, optional `tool_use_id`, and optional `workflow_run_id`. It exposes a relation for consumers such as ccmsg without merging child prose or tool streams into the parent; do not describe complete headless loading as globally pre-filtering sidechains.
- `--list-backup-sessions <absolute-payload-root> --provider <claude|codex|copilot>` and `--dump-backup-session <absolute-session-path> --backup-root <absolute-payload-root> --provider <...>` are the explicit read-only offline backup protocol advertised by `--capabilities`. They enumerate only recognized payload-relative provider carriers and parse metadata/messages through uncached path-specific loaders, without consulting current provider roots, live editor state, or ordinary provider caches. Both commands require regular absolute roots, reject symlinks and traversal component by component, canonicalize every accepted path beneath the payload root, and validate the provider before walking. Supported surfaces are Claude projects, Codex active/archived sessions, Copilot CLI/Desktop sessions, and Copilot VS Code workspace/global session stores. Keep this isolation boundary explicit; do not route backup browsing through ordinary `--list-sessions` or `--dump-session` resolution.
- Claude session metadata uses the same effective title as Claude Code: the latest manual `/rename` or `/branch` name wins, followed by the first non-empty `ai-title`, the legacy `type:"summary"` value, and only then the existing first-user/first-assistant/last-user preview fallbacks. Generated and legacy titles keep `is_renamed:false`. Both metadata phases capture `ai-title` before the conversational allowlist drops it, incremental/cache state retains it, and metadata cache version 18 retains the generated/fallback baseline plus chronological rename transitions. The optional provider-neutral `title_history` field lists trustworthy earlier effective titles oldest-first and excludes the current title. Claude reconstructs it from append-only rename/custom-title records, Codex combines preview/derived SQLite baselines with chronological `session_index.jsonl` names and the authoritative current SQLite state, and Copilot VS Code replays `customTitle` snapshot/patch records including resets. Unsupported or current-only providers serialize no history, and isolated Codex backup reads do not consult live external indexes or SQLite.
- `--list-sessions --provider codex [--output <file>]` joins each Codex thread id against `$CODEX_HOME/external_agent_session_imports.json`, Codex's authoritative external-agent import ledger. Imported rows gain `is_imported:true` and, when the private `source_path` identifies a known provider root, an `imported_from` label. It also exposes the optional `forked_from_id` recorded by the rollout's **first** `session_meta`; later replayed metadata in a fork cannot override either the child identity or its immediate parent. A downstream fork therefore preserves one direct edge per session (`child → parent → ancestor`) rather than flattening the lineage. String `source` values retain the existing provider-qualified entrypoint mapping, while the authoritative structured `source.subagent.thread_spawn` shape becomes `entrypoint:"codex-subagent"`; unknown structured variants remain unset. Paginated compound filenames encode a stable logical thread id before `_` and a distinct immutable rollout id after it. Unscoped Codex listing walks active and archived rollout roots once, groups canonical paginated carriers by logical id, selects the valid `state_5.sqlite.threads.rollout_path`, and otherwise uses deterministic filename order only when no applicable paginated database selection exists; native title resolution also follows that storage generation, preferring a paginated row's non-empty `threads.name` over its derived `title`, while legacy rows retain the distinct-title then `session_index.jsonl` compatibility precedence and older schemas without the new columns remain readable. The metadata cache is version 2 and persists rollout-derived `SessionInfo` keyed by root-relative carrier path plus high-resolution mtime and size. Cold parses are cached only when before/after fingerprints match; version/corruption/stat/write failures fail open to parsing, vanished or suppressed carriers are evicted, native titles/import-ledger state/archive-root provenance remain live overlays, and the cache is never continuity evidence for normalized message snapshots. Provider-wide cache replacement and targeted cache merging share a cross-process lock; a targeted stable miss re-reads the latest cache and rechecks its fingerprint/discoverability while locked, so concurrent targets do not evict one another. The optional headless-v1 `--session-metadata <session-id|session-path> --provider <name>` command emits one field-equivalent flattened listing row or JSON `null`; a selected paginated carrier remains a row and its superseded carrier returns `null`, which requires the Codex path to consult SQLite or scan canonical sibling filenames. Absolute Codex rollout paths and live Copilot CLI/Desktop `events.jsonl` or VS Code `chatSessions` carriers take provider-owned direct paths with exact lexical containment, symlink/reparse rejection, and canonical confinement. Canonical full Copilot UUIDs also take a targeted route: CLI/Desktop probes only `session-state/<id>/events.jsonl`, while VS Code enumerates workspace directory names and probes only `<id>.jsonl` in each workspace and empty-window store. The UUID is validated before path construction, exact candidates retain live workspace/classifier overlays and ordinary selector precedence, and a missing candidate falls back to the authoritative provider-wide listing; prefixes and noncanonical legacy ids remain wide by design. Copilot VS Code direct reads reuse a versioned high-resolution carrier cache whose provider-wide rebuilds and targeted one-entry merges share a cross-process lock, while `workspace.json` identity and `state.vscdb` orphan/archive/pin flags remain live. The private Codex import source path is never emitted; missing or malformed optional metadata degrades to an ordinary native session.
- Codex child-agent rows additionally expose optional `subagent_provenance:{spawned_at,agent_path,agent_nickname?}`. This object is derived only from the structurally recognized `source.subagent.thread_spawn` on the rollout's **first** `session_meta`; `spawned_at` is that record's top-level RFC 3339 timestamp, `agent_path` must be non-empty, and a malformed or blank nickname is omitted. Missing, malformed, unknown, or replayed metadata never fabricates provenance. The direct parent remains the separate `forked_from_id`, so consumers can join the child to its parent while using `spawned_at` to exclude inherited pre-spawn history.
- Copilot VS Code `--dump-session` emits one synthetic `subtype:"compact_summary"` record for each logical non-empty `requests[].result.metadata.summaries[]` entry. It collapses only entries with the same non-empty `toolCallRoundId` and byte-identical text; distinct ids, missing ids, or conflicting text remain separate in source order. The flat `metadata.summary` is used only when the structured array contains no non-empty text. This is dump-path normalization and does not require a metadata-cache version bump.
- Copilot VS Code hidden `copilot_replaceString` response parts can omit `invocationMessage.uris` even though `requests[].result.metadata.toolCallRounds[].toolCalls[]` retains the original `replace_string_in_file` arguments. `--dump-session` uses one uniquely correlated exact/native-suffixed call id as a fallback and projects only non-empty `filePath` into `tool_use.input.path`; the serialized URI remains authoritative when present, while missing, malformed, wrong-name, or ambiguous metadata fails closed. This is dump-path normalization and does not require a metadata-cache version bump.
- Copilot VS Code `copilot_findFiles` and `copilot_findTextInFiles` response parts retain only a prose `invocationMessage`, while `requests[].result.metadata.toolCallRounds[].toolCalls[]` retains `file_search.query` and `grep_search.query` / optional `includePattern` / optional `isRegexp`. `--dump-session` recovers only those allowlisted primitive fields from one uniquely correlated exact/native-suffixed call with the expected native name, accepting object or JSON-string arguments. Existing input fields remain authoritative; empty required fields, malformed arguments or optional types, wrong names, and ambiguous matches fail closed, while unrelated metadata fields are ignored. This changes only dump/snapshot normalization, so the provider metadata cache stays valid; an older Copilot snapshot whose retained normalized prefix differs safely returns a complete result through the existing prefix-digest check, so the cursor version remains unchanged.
- Copilot VS Code completed `toolInvocationSerialized` response parts can omit `pastTenseMessage` even though `requests[].result.metadata.toolCallResults` retains the authoritative result under the exact call id or its native `__vscode-*` suffix. `--dump-session` uses one uniquely correlated metadata entry only when the serialized result text is empty, accepts either one non-empty direct string or one flat serialized text-node tree, and emits at most one `tool_result` per call id even when VS Code retained duplicate invocation snapshots. Serialized result text remains authoritative; ambiguous ids, multiple content entries, nested nodes, line-break nodes, and unknown value shapes fail closed. This changes only dump/snapshot normalization, so the provider metadata cache stays valid; an older Copilot snapshot whose retained normalized prefix differs safely returns a complete result through the existing prefix-digest check, so the cursor version also remains unchanged.
- Copilot VS Code `completionTokens:0` on a non-empty visible assistant response is an unavailable-accounting sentinel used by some third-party model adapters, not authoritative zero usage. `--dump-session` omits `usage` for that response while preserving every positive recorded completion count. This is dump-path normalization and does not require a metadata-cache version bump.
- Codex `--dump-session` merges each result onto its matching tool call. When an `apply_patch` result reports the provider's known verification or invalid-patch failure prefix, the normalized `tool_result` gains `is_error:true`; successful results and similarly worded results from other tools remain unchanged.
- Codex `--dump-session` follows app-server's positive conversation projection. Legacy `event_msg.user_message` and paginated `event_msg.item_completed` records carrying `item.type:"UserMessage"` are authoritative visible input; the first canonical input in a provider turn gains `subtype:"authored_user"` and later canonical input in that turn gains `subtype:"steer"`. Ordinary user-role `response_item` records are model-history input and remain preserved as `subtype:"injected_context"`, never independently visible conversation turns. An exact latest-response text match may reuse the raw row to retain richer content, timestamps, or prompt-artifact metadata, but failed or ambiguous matching cannot hide the canonical event or promote an unmatched raw record. Canonical legacy events retain non-empty `client_id` as `data.clientMessageId`; explicit or active turn ownership is retained as `data.providerTurnId`; paginated events preserve their canonical item id, client id, content, and turn id. The sole raw user-role exception is the provider's exact XML `<hook_prompt hook_run_id="…">…</hook_prompt>` carrier: every content block must be `input_text`, every block must parse as the complete XML element with a non-empty hook id, and the normalized record uses `subtype:"hook_prompt"` plus safe fragment metadata. Wrapper lookalikes remain raw-only. Provider-turn lanes, terminal-context diagnostics, and quiescent snapshot checkpointing remain in place for enrichment and incremental replay, but ambiguity now affects only provenance diagnostics rather than default visibility. Snapshot cursor version 9 forces older clients through the existing safe full-reparse fallback. Session previews still prefer the first canonical user event while legacy event-less rollouts retain their first response-item fallback, and `message_count` semantics are unchanged.
- Codex paginated continuation loading treats `history_base.thread_id` as the preceding immutable rollout id, not another logical listing row. Live dumps recursively prepend only the validated `end_byte_offset` / `end_ordinal_exclusive` prefix from each active or archived ancestor, seed parser identity from the selected carrier before replaying that ancestry, and reject missing, ambiguous, cyclic, non-paginated, filename/metadata-mismatched, split-record, or ordinal-inconsistent lineages. Search iterates only selected logical rows, so neither duplicate carrier results nor post-cutoff superseded messages survive. Linked snapshots currently return cursorless full results; single-carrier snapshots retain cursor version 16 and their incremental replacement path. Offline backup reads remain confined to one supplied carrier and never resolve a missing ancestor from live storage.
- Codex `--dump-session` correlates the provider's usual top-level `compacted` record and later `event_msg.context_compacted` notification as one logical compaction. The authoritative record becomes one `compact_boundary`; the companion is suppressed only across the exact `world_state` / `turn_context` / `thread_settings_applied` / token-count bookkeeping sequence. A standalone notification remains `microcompact_boundary`, and unrelated activity cancels correlation. The same streaming rule applies to full and cursor-resumed parses.
- Codex `--dump-session` treats a cumulative token snapshot whose output total has not advanced as bookkeeping rather than a zero-token assistant invocation. Any input/cache delta remains pending and is combined with the next positive-output delta before being attached to the actual assistant record, so compaction and turn-boundary snapshots cannot create false visible zero-usage messages or lose their input-side accounting.
- Codex `--dump-session` adds a provenance-bearing `inference.cost` credit estimate to usage-bearing assistant records only when the rollout supplies a supported exact OpenAI model, a recognized standard/Fast service tier, and a non-empty ChatGPT `rate_limits.plan_type`. The calculation separates cached input from ordinary input, treats reasoning as part of output rather than charging it twice, maps persisted `priority` to configured Fast mode, and pins the published credit rates to `rateCardVersion:"2026-07-31"`. API-key/custom providers, unknown models or tiers, missing billing evidence, and unpriced features remain unset; the provider-reported `costUSD` field retains its independent authoritative meaning.
- User-authorship provenance advanced the opaque Codex snapshot cursor version to `3`; zero-output token-bookkeeping handling advanced it to `4`; prompt-artifact metadata advanced it to `5`; physical-turn/client provenance and strict terminal-sequence invalidation advanced it to `6`; interleaved developer context advanced it to `7`; overlapping-task authorship isolation advanced it to `8`; app-server-compatible positive user projection advanced it to `9`; provider-neutral interruption events advanced it to `10`; provider-rendered images advanced it through `12`; fork rollback provenance advanced it to `13`; confirmed zero-exclusion fork boundaries advanced it to `14`; asynchronous question reply metadata advanced it to `15`; compaction correlation across `thread_settings_applied` advanced it to `16`; authored local-image locator correction advances it to `17`. Older cursors fall back to a complete snapshot so retained prefixes normalized under an earlier contract are repaired rather than reused.
- Codex `--dump-session` also identifies the complete provider-generated pasted-text carrier (`# Files mentioned by the user` plus `.codex/attachments/<UUID>/pasted-text*.txt`) and adds safe `data.promptArtifacts:[{kind:"note",name,label}]` metadata to the verbatim user message. Partial mentions do not match; absolute paths, UUIDs, and external file contents are never copied into metadata or re-read.
