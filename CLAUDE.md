...
The headless `image-artifacts-v1` capability preserves two presentation proofs without treating arbitrary tool images as chat output. Final-Markdown images retain the existing exact provider-turn, tool-result byte identity, and local-path correlation chain; a narrowly parsed `Promise.all`/destructuring exec shape additionally maps each literal `tools.view_image` call to the corresponding `image(variable.image_url)` output, rejecting unmatched counts, duplicate variables, nonliteral paths, or any unrecognized JavaScript. Completed `event_msg.image_generation_end` records are normalized separately as `provider_rendered_image` Canvas artifacts whose base64 source, call ID, media identity, and presentation kind remain in one message. Because those events currently carry no turn ID, they are emitted only while exactly one non-ambiguous task owns the event; overlap omits them. Snapshot cursor version 12 invalidates retained prefixes that predate this normalized output.
