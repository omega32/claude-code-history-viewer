...
- Codex `--dump-session` classifies user-role `response_item` records from their structural pairing with `event_msg.user_message`, consistent authored turn metadata, and the provider's intervening `world_state` / `turn_context` input boundary, never from tag names. A positively paired first/ordinary prompt gains `subtype:"authored_user"`, later paired input in the active turn remains `subtype:"steer"`, and unmatched same-turn provider context before that boundary gains `subtype:"injected_context"`. Current resumed tasks may interleave same-turn developer-role instruction records between several user-role context carriers, and separate tasks may overlap physically in one rollout; pending candidates and authored identities are isolated by provider turn so activity or completion in one known task cannot invalidate, terminal-classify, or reset another. Turnless `user_message` events pair only with the physically latest pending response item by exact text, and a mismatch invalidates every ambiguous pending lane rather than searching backward. Turnless `world_state` follows the physically latest pending lane, while explicit `world_state`, `turn_context`, response-item, and completion records affect only their known turn. These records preserve the pending corridor only while every candidate is still before the input boundary, while an unknown-turn, mismatched-turn, or post-boundary record fails open. Positively paired inputs also retain the event's non-empty `client_id` as `data.clientMessageId`, while response items with explicit turn metadata retain `data.providerTurnId`; multiple authored inputs can therefore share one physical provider turn without changing presentation grouping. During an already-authored active turn, exactly one otherwise unresolved same-turn record can also become `injected_context` at a matching `task_complete` only after the explicit sequence candidate → input boundary → recognized same-turn agent activity → completion. Known duplicate/bookkeeping records are narrowly allowlisted, while EOF, malformed gaps, unknown valid records or events, reordered activity/boundaries, absent or mismatched boundaries, duplicate client ids, competing pending records, cross-turn provenance, and every other incomplete or ambiguous shape fail open as `subtype:"authorship_unknown"`; original content and existing `data` metadata are preserved in every case. Snapshot checkpoints advance only after every authorship task and pending lane is quiescent, so completing an overlapping task cannot strand an earlier unresolved candidate in the retained prefix. Session previews prefer the first authored user event while legacy event-less rollouts retain their first response-item fallback, and `message_count` semantics are unchanged.
