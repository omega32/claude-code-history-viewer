...
- Headless Copilot session metadata now resolves canonical full session IDs through exact CLI/Desktop and VS Code carrier candidates before falling back to the provider-wide listing, reducing ccmsg live refresh lookup time without changing prefix or legacy-ID behavior.

## [1.22.1] - 2026-09-22

### Fixed

- Codex authored prompt images now preserve local Windows and POSIX paths as path locators instead of URL locators across legacy and paginated carriers, including wrapper-mismatch turns whose raw and canonical user records cannot be merged.
...
- Codex image artifacts now retain local-path correlations for the exact result-array `Promise.all` pattern emitted through an ordered `for...of` image loop, while transformed, reordered, dynamic, or otherwise ambiguous JavaScript remains uncorrelated.

## [1.21.2] - 2026-09-08

### Fixed

- Codex paginated sessions now display their persisted explicit `threads.name` after a listing refresh instead of reverting to the stale derived `threads.title`; legacy databases and `session_index.jsonl` fallback behavior remain supported.
